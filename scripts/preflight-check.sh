...
if [ "$MISSING_SECRETS" -eq 1 ]; then
  echo "Warning: GitHub secrets are missing. They are optional for local sandbox loops."
  echo "If you need them later, add CLAUDE_CODE_OAUTH_TOKEN and GH_READ_TOKEN in .env and run: bash scripts/setup-github-secrets.sh"
fi
